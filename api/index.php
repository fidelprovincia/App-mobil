...
switch($method){
    case "GET": 
        ...
        // 🔹 Obtener atracciones visitadas por un usuario
            ...
            $sql = "SELECT DISTINCT l.atraccion_id AS atraccion_id, l.atraccion_nombre, l.image_url, l.provincia_id
                    FROM visited v
    ...
    case "POST":
        ...
        if (isset($path[3]) && $path[3] === "recomendar") {
        ...
        else if ($method === 'POST' && isset($path[2]) && $path[2] === "resenas") {
            $usuario_id = $input['usuario_id'] ?? null;
            $atraccion_id = $input['atraccion_id'] ?? null;
            $texto = trim($input['reseña'] ?? '');
            ...
            // ➤ Obtener la reseña con JOIN para devolver user_nombre
            $sql2 = "SELECT r.id, r.usuario_id, r.atraccion_id, r.reseña, r.estrellas, r.fecha,
                            u.name AS user_nombre
                    FROM reseñas r
                    JOIN users u ON u.id = r.usuario_id
                    WHERE r.id = :id
            ...
            $stmt2->execute();
            $nuevaResena = $stmt2->fetch(PDO::FETCH_ASSOC);

            echo json_encode($nuevaResena);
            return;
        }
        ...
        //UPDATE USER
        else if (isset($path[2]) && $path[2] === "update-user") {
            ...
            // Campos permitidos
            $allowed = ["name", "apellido", "pais"];

            if (!$id || !$field || !$value || !in_array($field, $allowed)) {
                echo json_encode([
                    "status" => 0,
                    "msg" => "Datos inválidos"
                ]);
            ...
            // Generar SQL dinámico (solo para columnas permitidas)
    ...
    case "DELETE":
        ...
        // Ruta: /api/users/{id}
        if (isset($path[2]) && $path[2] === "users" && isset($path[3])) {
            ob_clean();
            error_reporting(0); // desactiva warnings
            $user_id = intval($path[3]);
            ...
            if ($stmt->rowCount() === 0) {
                error_log("CHK-B6: User not found in database");
            ...
            error_log("CHK-B7: Deleting related records (visited, reseñas)");

            // Si quieres borrar cosas relacionadas (visitadas, reseñas)
            $conn->prepare("DELETE FROM visited WHERE usuario_id = :id")
                ->execute([':id' => $user_id]);

            $conn->prepare("DELETE FROM reseñas WHERE usuario_id = :id")
                ->execute([':id' => $user_id]);
        ...
        // =====================
            ...
            ob_clean();
            $resena_id = intval($path[3]);

            error_log("🗑️ DELETE REQUEST - Resena ID: " . $resena_id);

            // Verificar si existe
            $sql = "SELECT * FROM reseñas WHERE id = :id";
            $stmt = $conn->prepare($sql);
            $stmt->bindParam(":id", $resena_id, PDO::PARAM_INT);
            $stmt->execute();
            ...
            // Proceder a borrar
            $sqlDelete = "DELETE FROM reseñas WHERE id = :id";
            $stmtDelete = $conn->prepare($sqlDelete);

            if ($stmtDelete->execute([":id" => $resena_id])) {
                echo json_encode([
                    "status" => 1,
        ...
        else if ($method === "DELETE" && isset($path[2]) && $path[2] === "visitado") {
            ...
            $user_id = intval($path[3]);
            $atraccion_id = intval($path[4]);

            $sql = "DELETE FROM visited 
                    WHERE usuario_id = :user_id AND ciudad_id = :atraccion_id";

            $stmt = $conn->prepare($sql);

            if ($stmt->execute([":user_id" => $user_id, ":atraccion_id" => $atraccion_id])) {

                echo json_encode([
                    "status" => 1,
                    "message" => "Visitado eliminado"
                ]);
            } else {
                echo json_encode([
                    "status" => 0,
